...
SCRIPT_TEMPLATE = (
    "kepler -runwf -nogui -redirectgui {job.path} "
...
BATCH_TEMPLATE = "\n".join(
    [
        ...
        "kepler_load {cfg.system.kepler_load}",
        "{scripts}",
    ]
)
...
class Ets6System(AbstractSystem):
    ...
    def write_batchfile(self, run_dir: Path):
        ...
        batchfile = "\n".join(
            [
                "#!/bin/sh",
                f"#SBATCH -J duqtools.ets6.{run_dir.name}",
                f"#SBATCH -o {run_dir}/slurm.out",
                f"#SBATCH -e {run_dir}/slurm.err",
                "#SBATCH -p gw",
                "#SBATCH -N 1",
                "#SBATCH -n 1",
                "#SBATCH -t 1:00:00",
    ...
    def write_array_batchfile(
        self, jobs: Sequence[Job], max_jobs: int, max_array_size: int
    ):
        ...
        batchfile = "#!/bin/sh\n" + BATCH_TEMPLATE.format(cfg=self.cfg, scripts=scripts)
    ...
    def _submit_array(
        self, jobs: Sequence[Job], *, max_jobs: int, max_array_size: int, **kwargs
    ):
        for job in jobs:
            job.lockfile.touch()
