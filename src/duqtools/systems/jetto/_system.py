...
class BaseJettoSystem(AbstractSystem):
    ...
    def write_batchfile(self, run_dir: Path):
        ...
        _write_batchfile(run_dir, jetto_jset, cfg=self.cfg, jruns_path=self.jruns_path)
    ...
    def submit_job(self, job: Job):
        ...
        if submit_system == "slurm":
            ...
            raise NotImplementedError(
                f"submission type {submit_system}" " not implemented"
            )
    ...
    def submit_docker(self, job: Job):
        ...
        extra_volumes = {
            job.path.parent
            / "imasdb": {"bind": "/opt/imas/shared/imasdb", "mode": "rw"},
        }
    ...
    def submit_prominence(self, job: Job):
        ...
        cmd = [
            ...
            str(job.path),
            "--image",
            self.options.prominence_image,
            "--cpus",
            str(jetto_jset["JobProcessingPanel.numProcessors"]),
            "--walltime",
            "24.0",
            "--name",
            f"duqtools_{job.path.name}",
            "--cmd",
    ...
    def submit_array(
        ...
        **kwargs,
        ...
        if self.options.submit_system == "slurm":
            self.create_array_slurm(
                jobs, max_jobs=max_jobs, max_array_size=max_array_size
            )
            if not create_only:
            ...
            raise NotImplementedError(
                f"array submission type {self.options.submit_system}" " not implemented"
            )

    @add_to_op_queue(
        "Create array job", "Submit using `sbatch duqtools_slurm_array.sh`"
    )
    ...
    def copy_from_template(self, source_drc: Path, target_drc: Path):
        ...
        jetto_template = template.Template(
            jset=jetto_jset,
        ...
        lookup.to_file(
            jetto_lookup, target_drc / "lookup.json"
    ...
    def imas_from_path(self, template_drc: Path) -> ImasHandle:
        ...
        return ImasHandle(
            ...
            shot=jetto_jset["SetUpPanel.idsIMASDBShot"],
        )  # type: ignore
    ...
    def set_jetto_variable(
        self, run: Path, key: str, value, variable: Optional[JettoVar] = None
    ):
        jetto_template = template.from_directory(run)
