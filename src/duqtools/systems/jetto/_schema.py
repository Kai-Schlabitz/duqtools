...
class JettoSystemModel(SystemModel):
    ...
    submit_system: Literal["prominence", "slurm", "docker"] = Field(
        "slurm",
        description="System to submit jobs to " "[slurm (default), prominence, docker]",
    )
    docker_image: str = Field(
        "jintrac-imas", description="Docker image used for submission"
    )
    ...
    status_file: str = Field("jetto.status", description="Name of the status file.")
