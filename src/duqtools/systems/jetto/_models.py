...
class JsetField(BaseModel):
    file: Literal["jetto.jset"] = Field("jetto.jset", description="Name of the file.")
    field: str = Field(description="Field name.")


class NamelistField(BaseModel):
    file: Literal["jetto.in"] = Field("jetto.in", description="Name of the file.")
    field: str = Field(description="Field name.")
...
JettoField = Annotated[Union[JsetField, NamelistField], Field(discriminator="file")]
...
class JettoVar(BaseModel):
    ...
    keys: list[JettoField] = Field(
        description=f("Jetto keys to update when this jetto variable is requested")
    )
...
class JettoVariableModel(BaseModel):
    ...
    type: str = Field(
        "jetto-variable", description="Discriminator for the variable type."
    )
    ...
    lookup: Optional[JettoVar] = Field(
        None,
        description=f(
            """
...
class IDS2JettoVariableModel(BaseModel):
    ...
    type: str = Field(
        "IDS2jetto-variable", description="discriminator for the variable type"
    )
    paths: list[IDSPath] = Field(
        description=f(
            """
