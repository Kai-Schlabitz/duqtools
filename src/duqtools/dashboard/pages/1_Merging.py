...
df = pd.DataFrame.from_dict(
    {index: model.model_dump() for index, model in handles.items()}, orient="index"
)
...
with st.form("merge_form"):
    ...
    template = {
        "user": cols[0].text_input("User", value=a_run.user, key="user_template"),
        "db": cols[1].text_input("Machine", value=a_run.db, key="db_template"),
        "shot": cols[2].number_input("Shot", value=a_run.shot, key="shot_template"),
        "run": cols[3].number_input("Run", value=a_run.run, key="run_template"),
    }
    ...
    target = {
        ...
        "shot": cols[2].number_input("Shot", value=a_run.shot, key="shot_target"),
