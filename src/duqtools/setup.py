...
class ExtrasV210921:
    ...
    def __init__(self, template_file: str):
        ...
        self.run_numbers: dict[tuple[str, int], int] = defaultdict(lambda: 1000)
    ...
    def _get_n_samples(template_file: str) -> int:
        ...
        if not n_samples:
            raise ValueError(
                "`create.sampler.n_samples` must be defined for "
    ...
    def add_system_attrs(self, *, handle: ImasHandle, run: SimpleNamespace):
        ...
        self.run_numbers[handle.db, handle.shot] = data_out_start + self.n_samples

        if self.run_numbers[handle.db, handle.shot] > self.MAX_RUN:
            raise ValueError(f"Cannot write data with run number > {self.MAX_RUN}")
...
class Variables:
    ...
    def __getattr__(self, key: str):
        try:
            spec: IDS2JettoVariableModel = self.lookup[f"ids-{key}"]  # type: ignore
        except KeyError as exc:
            msg = (
                f"Cannot find {key!r} in your variable listing (i.e. `variables.yaml`)."
            )
        ...
        if value is None:
            raise AttributeError(f"No value matches specifications given by: {spec}")
    ...
    for name, handle in handles.items():
        ...
        if out_drc.exists() and not force:
            ...
            op_queue.add(
                ...
                kwargs={"drc": out_drc, "cfg": cfg, "force": force},
...
def setup(*, handle, template_file, run_name, force, **kwargs):
    ...
    substitute_templates(handles=handles, template_file=template_file, force=force)
